...
void toxav_callback_bwc (BWController *bwc, uint32_t friend_number, float loss, void *user_data);

int toxav_callback_invite(void *toxav_inst, MSICall *call);
int toxav_callback_start(void *toxav_inst, MSICall *call);
int toxav_callback_end(void *toxav_inst, MSICall *call);
int toxav_callback_error(void *toxav_inst, MSICall *call);
int toxav_callback_capabilites(void *toxav_inst, MSICall *call);

bool toxav_audio_bit_rate_invalid(uint32_t bit_rate);
bool toxav_video_bit_rate_invalid(uint32_t bit_rate);
bool toxav_invoke_call_state_callback(ToxAV *av, uint32_t friend_number, uint32_t state);
ToxAVCall *toxav_call_new(ToxAV *av, uint32_t friend_number, TOXAV_ERR_CALL *error);
ToxAVCall *toxav_call_get(ToxAV *av, uint32_t friend_number);
ToxAVCall *toxav_call_remove(ToxAVCall *call);
bool toxav_call_prepare_transmission(ToxAVCall *call);
void toxav_call_kill_transmission(ToxAVCall *call);
...
ToxAV *toxav_new(Tox *tox, TOXAV_ERR_NEW *error)
{
    ...
    msi_register_callback(av->msi, toxav_callback_invite, msi_OnInvite);
    msi_register_callback(av->msi, toxav_callback_start, msi_OnStart);
    msi_register_callback(av->msi, toxav_callback_end, msi_OnEnd);
    msi_register_callback(av->msi, toxav_callback_error, msi_OnError);
    msi_register_callback(av->msi, toxav_callback_error, msi_OnPeerTimeout);
    msi_register_callback(av->msi, toxav_callback_capabilites, msi_OnCapabilities);
...
void toxav_kill(ToxAV *av)
{
    ...
    if (av->calls) {
        ToxAVCall *it = toxav_call_get(av, av->calls_head);

        while (it) {
            toxav_call_kill_transmission(it);
            it = toxav_call_remove(it); /* This will eventually free av->calls */
        }
    }
                ...
                TOXAV_ERR_CALL *error)
{
    ...
    if ((audio_bit_rate && toxav_audio_bit_rate_invalid(audio_bit_rate))
            || (video_bit_rate && toxav_video_bit_rate_invalid(video_bit_rate))) {
        rc = TOXAV_ERR_CALL_INVALID_BIT_RATE;
        goto END;
    }

    ToxAVCall *av_call = toxav_call_new(av, friend_number, &rc);

    if (av_call == NULL)
        goto END;

    av_call->audio_bit_rate = audio_bit_rate;
    av_call->video_bit_rate = video_bit_rate;

    av_call->previous_self_capabilities = msi_CapRAudio | msi_CapRVideo;

    av_call->previous_self_capabilities |= audio_bit_rate > 0 ? msi_CapSAudio : 0;
    av_call->previous_self_capabilities |= video_bit_rate > 0 ? msi_CapSVideo : 0;

    if (msi_invite(av->msi, &av_call->msi_call, friend_number, av_call->previous_self_capabilities) != 0) {
        toxav_call_remove(av_call);
        rc = TOXAV_ERR_CALL_SYNC;
        goto END;
    }

    av_call->msi_call->av_call = av_call;
                  ...
                  TOXAV_ERR_ANSWER *error)
{
    ...
    ToxAVCall *av_call = toxav_call_get(av, friend_number);
    ...
    if (!toxav_call_prepare_transmission(av_call)) {
    ...
    if (msi_answer(av_call->msi_call, av_call->previous_self_capabilities) != 0)
...
bool toxav_call_control(ToxAV *av, uint32_t friend_number, TOXAV_CALL_CONTROL control, TOXAV_ERR_CALL_CONTROL *error)
{
    ...
    if (av_call == NULL || (!av_call->active && control != TOXAV_CALL_CONTROL_CANCEL)) {
    ...
    switch (control) {
        case TOXAV_CALL_CONTROL_RESUME: {
            ...
            if (av_call->msi_call->self_capabilities == 0 &&
                    av_call->previous_self_capabilities) {

                if (msi_change_capabilities(av_call->msi_call,
                                            av_call->previous_self_capabilities) == -1) {
                    rc = TOXAV_ERR_CALL_CONTROL_SYNC;
                    goto END;
                }

                rtp_allow_receiving(av_call->audio.first);
                rtp_allow_receiving(av_call->video.first);
            } else {
        ...
        case TOXAV_CALL_CONTROL_PAUSE: {
            ...
            if (av_call->msi_call->self_capabilities) {
                av_call->previous_self_capabilities = av_call->msi_call->self_capabilities;

                if (msi_change_capabilities(av_call->msi_call, 0) == -1) {
                    rc = TOXAV_ERR_CALL_CONTROL_SYNC;
                    goto END;
                }

                rtp_stop_receiving(av_call->audio.first);
                rtp_stop_receiving(av_call->video.first);
            } else {
                rc = TOXAV_ERR_CALL_CONTROL_INVALID_TRANSITION;
                goto END;
            }
        ...
        case TOXAV_CALL_CONTROL_CANCEL: {
            ...
            if (av_call->active)
            {
                pthread_mutex_lock(av_call->mutex);

                if (msi_hangup(av_call->msi_call) != 0) {
                    rc = TOXAV_ERR_CALL_CONTROL_SYNC;
                }

                av_call->msi_call = NULL;
                pthread_mutex_unlock(av_call->mutex);
            }
            else 
            {
                /* no mutexes prepared yet */
                if (msi_hangup(av_call->msi_call) != 0) {
                    rc = TOXAV_ERR_CALL_CONTROL_SYNC;
                }
                av_call->msi_call = NULL;
            ...
            toxav_call_kill_transmission(av_call);
            toxav_call_remove(av_call);
        }
        break;

        case TOXAV_CALL_CONTROL_MUTE_AUDIO: {
            if (av_call->msi_call->self_capabilities & msi_CapRAudio) {
                if (msi_change_capabilities(av_call->msi_call, 
                                            av_call->msi_call->self_capabilities ^ msi_CapRAudio) == -1) {
                    rc = TOXAV_ERR_CALL_CONTROL_SYNC;
                    goto END;
                }
        ...
        case TOXAV_CALL_CONTROL_HIDE_VIDEO: {
            if (av_call->msi_call->self_capabilities & msi_CapRVideo) {
                if (msi_change_capabilities(av_call->msi_call, 
                                            av_call->msi_call->self_capabilities ^ msi_CapRVideo) == -1) {
                    rc = TOXAV_ERR_CALL_CONTROL_SYNC;
                    goto END;
                }
                        ...
                        int32_t video_bit_rate, TOXAV_ERR_BIT_RATE_SET *error)
{
    ...
    if (audio_bit_rate > 0 && toxav_audio_bit_rate_invalid(audio_bit_rate)) {
        rc = TOXAV_ERR_BIT_RATE_SET_INVALID_AUDIO_BIT_RATE;
        goto END;
    }

    if (video_bit_rate > 0 && toxav_video_bit_rate_invalid(video_bit_rate)) {
        rc = TOXAV_ERR_BIT_RATE_SET_INVALID_VIDEO_BIT_RATE;
        goto END;
    }

    pthread_mutex_lock(av->mutex);
    av_call = toxav_call_get(av, friend_number);

    if (av_call == NULL || !av_call->active || av_call->msi_call->state != msi_CallActive) {
        pthread_mutex_unlock(av->mutex);
    ...
    if (audio_bit_rate >= 0) {
        ...
        if (av_call->audio_bit_rate == audio_bit_rate) {
            ...
            av_call->audio_bit_rate = 0;
        } else {
            pthread_mutex_lock(av_call->mutex);

            if (av_call->audio_bit_rate == 0) {
    ...
    if (video_bit_rate >= 0) {
        ...
        if (av_call->video_bit_rate == video_bit_rate) {
            ...
            if (msi_change_capabilities(av_call->msi_call, 
                                        av_call->msi_call->self_capabilities ^ msi_CapSVideo) != 0) {
                pthread_mutex_unlock(av->mutex);
                rc = TOXAV_ERR_BIT_RATE_SET_SYNC;
                goto END;
            }

            av_call->video_bit_rate = 0;
        } else {
            pthread_mutex_lock(av_call->mutex);

            if (av_call->video_bit_rate == 0) {
                            ...
                            uint8_t channels, uint32_t sampling_rate, TOXAV_ERR_SEND_FRAME *error)
{
    ...
    if (av_call->audio_bit_rate == 0 ||
            !(av_call->msi_call->self_capabilities & msi_CapSAudio) ||
            !(av_call->msi_call->peer_capabilities & msi_CapRAudio)) {
        pthread_mutex_unlock(av->mutex);
        rc = TOXAV_ERR_SEND_FRAME_PAYLOAD_TYPE_DISABLED;
        goto END;
    }

    pthread_mutex_lock(av_call->mutex_audio);
    pthread_mutex_unlock(av->mutex);

    if (pcm == NULL) {
        pthread_mutex_unlock(av_call->mutex_audio);
    ...
    { /* Encode and send */
        if (ac_reconfigure_encoder(av_call->audio.second, av_call->audio_bit_rate * 1000, sampling_rate, channels) != 0) {
            pthread_mutex_unlock(av_call->mutex_audio);
            rc = TOXAV_ERR_SEND_FRAME_INVALID;
            goto END;
        }

        C99_DYNARRAY( uint8_t, dest, sample_count + sizeof(sampling_rate) ); // -C99

        sampling_rate = htonl(sampling_rate);
        memcpy(dest, &sampling_rate, sizeof(sampling_rate));
        int vrc = opus_encode(av_call->audio.second->encoder, pcm, sample_count,
                              dest + sizeof(sampling_rate), C99_sizeof(dest) - sizeof(sampling_rate));
        ...
        if (rtp_send_data(av_call->audio.first, dest, vrc + sizeof(sampling_rate)) != 0) {
                            ...
                            const uint8_t *u, const uint8_t *v, TOXAV_ERR_SEND_FRAME *error)
{
    ...
    if (av_call->video_bit_rate == 0 ||
            !(av_call->msi_call->self_capabilities & msi_CapSVideo) ||
            !(av_call->msi_call->peer_capabilities & msi_CapRVideo)) {
        pthread_mutex_unlock(av->mutex);
        rc = TOXAV_ERR_SEND_FRAME_PAYLOAD_TYPE_DISABLED;
        goto END;
    }

    pthread_mutex_lock(av_call->mutex_video);
    pthread_mutex_unlock(av->mutex);

    if (y == NULL || u == NULL || v == NULL) {
        pthread_mutex_unlock(av_call->mutex_video);
        rc = TOXAV_ERR_SEND_FRAME_NULL;
        goto END;
    }

    if (vc_reconfigure_encoder(av_call->video.second, av_call->video_bit_rate * 1000, width, height) != 0) {
        pthread_mutex_unlock(av_call->mutex_video);
        rc = TOXAV_ERR_SEND_FRAME_INVALID;
        goto END;
    }
    ...
    { /* Encode */
        ...
        int vrc = vpx_codec_encode(av_call->video.second->encoder, &img,
                                   av_call->video.second->frame_counter, 1, 0, MAX_ENCODE_TIME_US);
    ...
    ++av_call->video.second->frame_counter;

    { /* Send frames */
        vpx_codec_iter_t iter = NULL;
        const vpx_codec_cx_pkt_t *pkt;

        while ((pkt = vpx_codec_get_cx_data(av_call->video.second->encoder, &iter))) {
            if (pkt->kind == VPX_CODEC_CX_FRAME_PKT &&
                    rtp_send_data(av_call->video.first, pkt->data.frame.buf, pkt->data.frame.sz) < 0) {
...
void toxav_callback_bwc(BWController *bwc, uint32_t friend_number, float loss, void *user_data)
{
    ...
    pthread_mutex_lock(av_call->av->mutex);

    if (!av_call->av->bcb.first) {
        pthread_mutex_unlock(av_call->av->mutex);
        LOGGER_WARNING("No callback to report loss on");
        return;
    }

    if (av_call->video_bit_rate)
        (*av_call->av->bcb.first) (av_call->av, friend_number, 
                                   av_call->audio_bit_rate,
                                   av_call->video_bit_rate - (av_call->video_bit_rate * loss),
                                   av_call->av->bcb.second);
    else if (av_call->audio_bit_rate)
        (*av_call->av->bcb.first) (av_call->av, friend_number,
                                   av_call->audio_bit_rate - (av_call->audio_bit_rate * loss),
                                   0, 
                                   av_call->av->bcb.second);
...
int toxav_callback_invite(void *toxav_inst, MSICall *call)
{
    ...
    ToxAVCall *av_call = toxav_call_new(toxav, call->friend_number, NULL);
...
int toxav_callback_start(void *toxav_inst, MSICall *call)
{
    ...
    ToxAVCall *av_call = toxav_call_get(toxav, call->friend_number);
    ...
    if (!toxav_call_prepare_transmission(av_call)) {
        toxav_callback_error(toxav_inst, call);
        pthread_mutex_unlock(toxav->mutex);
        return -1;
    }

    if (!toxav_invoke_call_state_callback(toxav, call->friend_number, call->peer_capabilities)) {
        toxav_callback_error(toxav_inst, call);
        pthread_mutex_unlock(toxav->mutex);
        return -1;
    }
...
int toxav_callback_end(void *toxav_inst, MSICall *call)
{
    ...
    toxav_invoke_call_state_callback(toxav, call->friend_number, TOXAV_FRIEND_CALL_STATE_FINISHED);

    if (call->av_call) {
        toxav_call_kill_transmission(call->av_call);
        toxav_call_remove(call->av_call);
    }
...
int toxav_callback_error(void *toxav_inst, MSICall *call)
{
    ...
    toxav_invoke_call_state_callback(toxav, call->friend_number, TOXAV_FRIEND_CALL_STATE_ERROR);
...
int toxav_callback_capabilites(void *toxav_inst, MSICall *call)
{
    ...
    toxav_invoke_call_state_callback(toxav, call->friend_number, call->peer_capabilities);
...
ToxAVCall *toxav_call_new(ToxAV *av, uint32_t friend_number, TOXAV_ERR_CALL *error)
{
    ...
    if (toxav_call_get(av, friend_number) != NULL) {
        rc = TOXAV_ERR_CALL_FRIEND_ALREADY_IN_CALL;
        goto END;
    }


    av_call = calloc(1, sizeof(ToxAVCall));
    ...
    av_call->av = av;
    av_call->friend_number = friend_number;

    if (av->calls == NULL) { /* Creating */
        av->calls = calloc (friend_number + 1, sizeof(ToxAVCall *));
        ...
        av_call->prev = av->calls[av->calls_tail];
        av->calls[av->calls_tail]->next = av_call;

        av->calls_tail = friend_number;

    } else if (av->calls_head > friend_number) { /* Inserting at front */
        av_call->next = av->calls[av->calls_head];
        av->calls[av->calls_head]->prev = av_call;
        av->calls_head = friend_number;
    }

    av->calls[friend_number] = av_call;
...
ToxAVCall *toxav_call_remove(ToxAVCall *av_call)
{
    if (av_call == NULL)
        return NULL;

    uint32_t friend_number = av_call->friend_number;
    ToxAV *av = av_call->av;

    ToxAVCall *prev = av_call->prev;
    ToxAVCall *next = av_call->next;

    /* Set av call in msi to NULL in order to know if call if ToxAVCall is
     * removed from the msi call.
     */
    if (av_call->msi_call) {
        av_call->msi_call->av_call = NULL;
    }
...
bool toxav_call_prepare_transmission(ToxAVCall *av_call)
{
    /* Assumes mutex locked */
    ...
    if (create_recursive_mutex(av_call->mutex_audio) != 0)
        return false;

    if (create_recursive_mutex(av_call->mutex_video) != 0)
        goto FAILURE_3;

    if (create_recursive_mutex(av_call->mutex) != 0)
        goto FAILURE_2;

    /* Prepare bwc */
    av_call->bwc = bwc_new(av->m, av_call->friend_number, toxav_callback_bwc, av_call);

    { /* Prepare audio */
        av_call->audio.second = ac_new(av, av_call->friend_number, av->acb.first, av->acb.second);

        if (!av_call->audio.second) {
            LOGGER_ERROR("Failed to create audio codec session");
            goto FAILURE;
        }

        av_call->audio.first = rtp_new(rtp_TypeAudio, av->m, av_call->friend_number, av_call->bwc,
                                       av_call->audio.second, ac_queue_message);

        if (!av_call->audio.first) {
            LOGGER_ERROR("Failed to create audio rtp session");;
            goto FAILURE;
        }
    }
    { /* Prepare video */
        av_call->video.second = vc_new(av, av_call->friend_number, av->vcb.first, av->vcb.second);

        if (!av_call->video.second) {
            LOGGER_ERROR("Failed to create video codec session");
            goto FAILURE;
        }

        av_call->video.first = rtp_new(rtp_TypeVideo, av->m, av_call->friend_number, av_call->bwc,
                                       av_call->video.second, vc_queue_message);

        if (!av_call->video.first) {
...
FAILURE:
    bwc_kill(av_call->bwc);
    rtp_kill(av_call->audio.first);
    ac_kill(av_call->audio.second);
    av_call->audio.first = NULL;
    av_call->audio.second = NULL;
    rtp_kill(av_call->video.first);
    vc_kill(av_call->video.second);
    av_call->video.first = NULL;
    av_call->video.second = NULL;
    pthread_mutex_destroy(av_call->mutex);
FAILURE_2:
    pthread_mutex_destroy(av_call->mutex_video);
FAILURE_3:
    pthread_mutex_destroy(av_call->mutex_audio);
    return false;
}
void toxav_call_kill_transmission(ToxAVCall *av_call)
{
    if (av_call == NULL || av_call->active == 0)
        return;
