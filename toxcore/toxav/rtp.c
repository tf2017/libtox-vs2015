...
int rtp_send_data (RTPSession *session, const uint8_t *data, uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, rdata, length + sizeof(struct RTPHeader) + 1 ); // -C99
    memset(rdata, 0, C99_sizeof(rdata));
    ...
    if (MAX_CRYPTO_DATA_SIZE > length + sizeof(struct RTPHeader) + 1) {
        ...
        if (-1 == send_custom_lossy_packet(session->m, session->friend_number, rdata, C99_sizeof(rdata)))
            LOGGER_WARNING("RTP send failed (len: %d)! std error: %s", C99_sizeof(rdata), strerror(errno));
    } else {
...
struct RTPMessage *new_message (size_t allocate_len, const uint8_t *data, uint16_t data_length)
{
    ...
    struct RTPMessage *msg = calloc(1, sizeof(struct RTPMessage) + (allocate_len - sizeof(struct RTPHeader)));
