...
static Group_JitterBuffer *create_queue(unsigned int capacity)
{
    ...
    if (!(q = calloc(1, sizeof(Group_JitterBuffer)))) return NULL;

    if (!(q->queue = calloc(size, sizeof(Group_Audio_Packet *)))) {
        free(q);
        return NULL;
    }
...
static int send_audio_packet(Group_Chats *g_c, int groupnumber, uint8_t *packet, uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, data, 1 + sizeof(uint16_t) + length ); // -C99
    ...
    if (send_group_lossy_packet(g_c, groupnumber, data, C99_sizeof(data)) == -1)
        return -1;
