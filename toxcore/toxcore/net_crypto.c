                                 ...
                                 uint8_t *shared_key)
{
    ...
    uint8_t padding[crypto_box_PUBLICKEYBYTES]; // -C99 = {0};
	sodium_memzero(padding, sizeof(padding));
...
static int send_data_packet(Net_Crypto *c, int crypt_connection_id, const uint8_t *data, uint16_t length)
{
    ...
    pthread_mutex_lock(&conn->mutex);
    C99_DYNARRAY( uint8_t, packet, 1 + sizeof(uint16_t) + length + crypto_box_MACBYTES ); // -C99
    ...
    if (len + 1 + sizeof(uint16_t) != C99_sizeof(packet)) {
        pthread_mutex_unlock(&conn->mutex);
        return -1;
    }
    ...
    return send_packet_to(c, crypt_connection_id, packet, C99_sizeof(packet));
}
                                   ...
                                   const uint8_t *data, uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, packet, sizeof(uint32_t) + sizeof(uint32_t) + padding_length + length ); // -C99
    ...
    return send_data_packet(c, crypt_connection_id, packet, C99_sizeof(packet));
}
