...
static int handle_data_request(void *object, IP_Port source, const uint8_t *packet, uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, data, length - (crypto_box_PUBLICKEYBYTES + ONION_RETURN_3) ); // -C99
    ...
    if (send_onion_response(onion_a->net, onion_a->entries[index].ret_ip_port, data, C99_sizeof(data),
