                           ...
                           uint32_t *message_id)
{
    ...
    C99_DYNARRAY( uint8_t, packet, length + 1 ); // -C99
                            ...
                            uint64_t filesize, const uint8_t *file_id, const uint8_t *filename, uint16_t filename_length)
{
    ...
    C99_DYNARRAY( uint8_t, packet, 1 + sizeof(file_type) + sizeof(filesize) + FILE_ID_LENGTH + filename_length ); // -C99
    packet[0] = filenumber;
    ...
    return write_cryptpacket_id(m, friendnumber, PACKET_ID_FILE_SENDREQUEST, packet, C99_sizeof(packet), 0);
}
                             ...
                             uint8_t control_type, uint8_t *data, uint16_t data_length)
{
    ...
    C99_DYNARRAY( uint8_t, packet, 3 + data_length ); // -C99
    ...
    return write_cryptpacket_id(m, friendnumber, PACKET_ID_FILE_CONTROL, packet, C99_sizeof(packet), 0);
}
                                     ...
                                     uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, packet, 2 + length ); // -C99
    ...
    return write_cryptpacket(m->net_crypto, friend_connection_crypt_connection_id(m->fr_c,
                             m->friendlist[friendnumber].friendcon_id), packet, C99_sizeof(packet), 1);
}
...
static int handle_packet(void *object, int i, uint8_t *temp, uint16_t len)
{
    ...
    switch (packet_id) {
        ...
        case PACKET_ID_NICKNAME: {
            ...
            C99_DYNARRAY( uint8_t, data_terminated, data_length + 1 ); // -C99
        ...
        case PACKET_ID_ACTION: {
            ...
            /* Make sure the NULL terminator is present. */
            C99_DYNARRAY( uint8_t, message_terminated, message_length + 1 ); // -C99
        ...
        case PACKET_ID_FILE_SENDREQUEST: {
            ...
            C99_DYNARRAY( uint8_t, filename_terminated, filename_length + 1 ); // -C99
...
void do_messenger(Messenger *m)
{
    ...
    if (unix_time() > lastdump + DUMPING_CLIENTS_FRIENDS_EVERY_N_SECONDS) {
        ...
        C99_DYNARRAY( int32_t, m2dht, num_dhtfriends ); // -C99
        C99_DYNARRAY( int32_t, dht2m, num_dhtfriends ); // -C99
