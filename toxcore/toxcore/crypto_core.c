                           ...
                           uint8_t *encrypted)
{
    ...
    C99_DYNARRAY( uint8_t, temp_plain, length + crypto_box_ZEROBYTES ); // -C99
    C99_DYNARRAY( uint8_t, temp_encrypted, length + crypto_box_MACBYTES + crypto_box_BOXZEROBYTES ); // -C99
                           ...
                           uint8_t *plain)
{
    ...
    C99_DYNARRAY( uint8_t, temp_plain, length + crypto_box_ZEROBYTES ); // -C99
    C99_DYNARRAY( uint8_t, temp_encrypted, length + crypto_box_BOXZEROBYTES ); // -C99
...
void increment_nonce_number(uint8_t *nonce, uint32_t host_order_num)
{
    ...
    uint8_t num_as_nonce[crypto_box_NONCEBYTES]; // -C99 = {0};
	sodium_memzero(num_as_nonce, sizeof(num_as_nonce));
