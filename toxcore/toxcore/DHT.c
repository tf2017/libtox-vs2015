                    ...
                    const Node_format *sendback_node)
{
    ...
    uint8_t plain_message[sizeof(Node_format) * 2]; // -C99 = {0};
	sodium_memzero(plain_message, sizeof(plain_message));
                          ...
                          const uint8_t *sendback_data, uint16_t length, const uint8_t *shared_encryption_key)
{
    ...
    C99_DYNARRAY( uint8_t, data, 1 + crypto_box_PUBLICKEYBYTES + crypto_box_NONCEBYTES + Node_format_size * MAX_SENT_NODES + length + crypto_box_MACBYTES ); // -C99

    Node_format nodes_list[MAX_SENT_NODES];
    uint32_t num_nodes = get_close_nodes(dht, client_id, nodes_list, 0, LAN_ip(ip_port.ip) == 0, 1);

    C99_DYNARRAY( uint8_t, plain, 1 + Node_format_size * MAX_SENT_NODES + length ); // -C99
    C99_DYNARRAY( uint8_t, encrypt, C99_sizeof(plain) + crypto_box_MACBYTES ); // -C99
                                 ...
                                 Node_format *plain_nodes, uint16_t size_plain_nodes, uint32_t *num_nodes_out)
{
    ...
    C99_DYNARRAY( uint8_t, plain, 1 + data_size + sizeof(uint64_t) ); // -C99
    uint8_t shared_key[crypto_box_BEFORENMBYTES];
    ...
    if ((unsigned int)len != C99_sizeof(plain))
        return 1;
        ...
        Client_data *list, uint32_t list_count, uint32_t *bootstrap_times, _Bool sortable)
{
    ...
    C99_DYNARRAY( Client_data *, client_list, list_count * 2 ); // -C99
    C99_DYNARRAY( IPPTsPng    *, assoc_list, list_count * 2 ); // -C99
    unsigned int sort = 0;
    _Bool sort_ok = 0;
...
int route_tofriend(const DHT *dht, const uint8_t *friend_id, const uint8_t *packet, uint16_t length)
{
    ...
    uint32_t i, sent = 0;
    uint8_t friend_sent[MAX_FRIEND_CLIENTS]; // -C99 = {0};
	sodium_memzero(friend_sent, sizeof(friend_sent));
...
static IP NAT_commonip(IP_Port *ip_portlist, uint16_t len, uint16_t min_num)
{
    ...
    uint32_t i, j;
    uint16_t numbers[MAX_FRIEND_CLIENTS]; // -C99 = {0};
...
static int send_hardening_req(DHT *dht, Node_format *sendto, uint8_t type, uint8_t *contents, uint16_t length)
{
    ...
    uint8_t packet[MAX_CRYPTO_REQUEST_SIZE];
    uint8_t data[HARDREQ_DATA_SIZE]; // -C99 = {0};
                                      ...
                                      const uint8_t *nodes_data, uint16_t nodes_data_length)
{
    ...
    uint8_t packet[MAX_CRYPTO_REQUEST_SIZE];
    C99_DYNARRAY( uint8_t, data, 1 + crypto_box_PUBLICKEYBYTES + nodes_data_length ); // -C99
    ...
    int len = create_request(dht->self_public_key, dht->self_secret_key, packet, sendto->public_key, data,
                             C99_sizeof(data), CRYPTO_PACKET_HARDENING);
