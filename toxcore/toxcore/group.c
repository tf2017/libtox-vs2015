        ...
        uint16_t group_num, const uint8_t *data, uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, packet, 1 + sizeof(uint16_t) + length ); // -C99
    packet[0] = packet_id;
...
static unsigned int send_peers(Group_Chats *g_c, int groupnumber, int friendcon_id, uint16_t group_num)
{
    ...
    if (g->title_len) {
        C99_DYNARRAY( uint8_t, Packet, 1 + g->title_len ); // -C99
        Packet[0] = PEER_TITLE_ID;
        memcpy(Packet + 1, g->title, g->title_len);
        send_packet_group_peer(g_c->fr_c, friendcon_id, PACKET_ID_DIRECT_GROUPCHAT, group_num, Packet, C99_sizeof(Packet));
    }
                                       ...
                                       uint16_t len)
{
    ...
    C99_DYNARRAY( uint8_t, packet, sizeof(uint16_t) + sizeof(uint32_t) + 1 + len ); // -C99
    ...
    return send_message_all_close(g_c, groupnumber, packet, C99_sizeof(packet), -1);
}
...
int send_group_lossy_packet(const Group_Chats *g_c, int groupnumber, const uint8_t *data, uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, packet, sizeof(uint16_t) * 2 + length ); // -C99
    ...
    if (send_lossy_all_close(g_c, groupnumber, packet, C99_sizeof(packet), -1) == 0) {
        return -1;
    }
                                        ...
                                        int close_index)
{
    ...
    switch (message_id) {
        ...
        case PACKET_ID_MESSAGE: {
            ...
            C99_DYNARRAY( uint8_t, newmsg, msg_data_len + 1 ); // -C99
