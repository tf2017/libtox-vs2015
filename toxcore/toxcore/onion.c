                        ...
                        const uint8_t *data, uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, step1, SIZE_IPPORT + length ); // -C99
    ...
    len = encrypt_data_symmetric(path->shared_key1, nonce, step3, C99_sizeof(step3),
...
int send_onion_response(Networking_Core *net, IP_Port dest, const uint8_t *data, uint16_t length, const uint8_t *ret)
{
    ...
    C99_DYNARRAY( uint8_t, packet, 1 + RETURN_3 + length ); // -C99
    packet[0] = NET_PACKET_ONION_RECV_3;
    memcpy(packet + 1, ret, RETURN_3);
    memcpy(packet + 1 + RETURN_3, data, length);

    if ((uint32_t)sendpacket(net, dest, packet, C99_sizeof(packet)) != C99_sizeof(packet))
        return -1;
