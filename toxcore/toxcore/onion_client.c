                                        ...
                                        const uint8_t *ping_id, uint32_t pathnum)
{
    ...
    uint8_t zero_ping_id[ONION_PING_ID_SIZE]; // -C99 = {0};
	sodium_memzero(zero_ping_id, sizeof(zero_ping_id));
...
static int handle_announce_response(void *object, IP_Port source, const uint8_t *packet, uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, plain, 1 + ONION_PING_ID_SIZE + len_nodes ); // -C99
    int len = -1;
    ...
    if ((uint32_t)len != C99_sizeof(plain))
        return 1;
...
static int handle_data_response(void *object, IP_Port source, const uint8_t *packet, uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, temp_plain, length - ONION_DATA_RESPONSE_MIN_SIZE ); // -C99
    ...
    if ((uint32_t)len != C99_sizeof(temp_plain))
        return 1;

    C99_DYNARRAY( uint8_t, plain, C99_sizeof(temp_plain) - DATA_IN_RESPONSE_MIN_SIZE ); // -C99
    len = decrypt_data(temp_plain, onion_c->c->self_secret_key, packet + 1, temp_plain + crypto_box_PUBLICKEYBYTES,
                       C99_sizeof(temp_plain) - crypto_box_PUBLICKEYBYTES, plain);
...
static int handle_tcp_onion(void *object, const uint8_t *data, uint16_t length)
{
    ...
    IP_Port ip_port; // -C99 = {0};
	sodium_memzero(&ip_port, sizeof(ip_port));
...
int send_onion_data(Onion_Client *onion_c, int friend_num, const uint8_t *data, uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, packet, DATA_IN_RESPONSE_MIN_SIZE + length ); // -C99
    ...
    if ((uint32_t)len + crypto_box_PUBLICKEYBYTES != C99_sizeof(packet))
        return -1;
    ...
    for (i = 0; i < num_good; ++i) {
        ...
        len = create_data_request(o_packet, sizeof(o_packet), onion_c->friends_list[friend_num].real_public_key,
                                  list_nodes[good_nodes[i]].data_public_key, nonce, packet, C99_sizeof(packet));
...
static int send_dht_dhtpk(const Onion_Client *onion_c, int friend_num, const uint8_t *data, uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, temp, DATA_IN_RESPONSE_MIN_SIZE + crypto_box_NONCEBYTES + length ); // -C99
    ...
    if ((uint32_t)len + crypto_box_PUBLICKEYBYTES + crypto_box_NONCEBYTES != C99_sizeof(temp))
        return -1;

    uint8_t packet[MAX_CRYPTO_REQUEST_SIZE];
    len = create_request(onion_c->dht->self_public_key, onion_c->dht->self_secret_key, packet,
                         onion_c->friends_list[friend_num].dht_public_key, temp, C99_sizeof(temp), CRYPTO_PACKET_DHTPK);
