...
{
    struct sockaddr_storage addr; // -C99 = { 0 }; 
                                      ...
                                      uint8_t *recv_nonce, uint8_t *data, uint16_t max_len)
{
    ...
    C99_DYNARRAY( uint8_t, data_encrypted, *next_packet_length ); // -C99
        ...
        _Bool priority)
{
    ...
    C99_DYNARRAY( uint8_t, packet, sizeof(uint16_t) + length + crypto_box_MACBYTES ); // -C99

    uint16_t c_length = htons(length + crypto_box_MACBYTES);
    memcpy(packet, &c_length, sizeof(uint16_t));
    int len = encrypt_data_symmetric(con->shared_key, con->sent_nonce, data, length, packet + sizeof(uint16_t));

    if ((unsigned int)len != (C99_sizeof(packet) - sizeof(uint16_t)))
        return -1;

    if (priority) {
        len = sendpriority ? send(con->sock, packet, C99_sizeof(packet), MSG_NOSIGNAL) : 0;
        ...
        if ((unsigned int)len == C99_sizeof(packet)) {
            return 1;
        }

        return add_priority(con, packet, C99_sizeof(packet), len);
    }

    len = send(con->sock, packet, C99_sizeof(packet), MSG_NOSIGNAL);
    ...
    memcpy(con->last_packet, packet, C99_sizeof(packet));
    con->last_packet_length = C99_sizeof(packet);
    con->last_packet_sent = len;
    return 1;
}
                               ...
                               uint16_t length)
{
    ...
    if (other_index != -1) {
        C99_DYNARRAY( uint8_t, resp_packet, 1 + crypto_box_PUBLICKEYBYTES + length ); // -C99
...
static int handle_onion_recv_1(void *object, IP_Port dest, const uint8_t *data, uint16_t length)
{
    ...
    C99_DYNARRAY( uint8_t, packet, 1 + length ); // -C99
    memcpy(packet + 1, data, length);
    packet[0] = TCP_PACKET_ONION_RESPONSE;

    if (write_packet_TCP_secure_connection(con, packet, C99_sizeof(packet), 0) != 1)
        return 1;
...
static int handle_TCP_packet(TCP_Server *TCP_server, uint32_t con_id, const uint8_t *data, uint16_t length)
{
    ...
    switch (data[0]) {
        ...
        default: {
            ...
            C99_DYNARRAY( uint8_t, new_data, length ); // -C99
