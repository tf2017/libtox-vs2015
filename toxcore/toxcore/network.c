...
void kill_sock(sock_t sock)
{
	...
	if (shutdown(sock, SD_BOTH) == SOCKET_ERROR)
		LOGGER_ERROR("shutdown error: %d", WSAGetLastError());
	if (closesocket(sock) == SOCKET_ERROR)
        LOGGER_ERROR("closesocket error: %d", WSAGetLastError());
#else
	shutdown(sock, 2);
	close(sock);
#endif
...
static int receivepacket(sock_t sock, IP_Port *ip_port, uint8_t *data, uint32_t *length)
{
    ...
    *length = (uint32_t)fail_or_len;
    ...
    if (addr.ss_family == AF_INET) {
        ...
        ip_port->ip.family = (uint8_t)addr_in->sin_family;
        ip_port->ip.ip4.in_addr = addr_in->sin_addr;
        ip_port->port = addr_in->sin_port;
    } else if (addr.ss_family == AF_INET6) {
        struct sockaddr_in6 *addr_in6 = (struct sockaddr_in6 *)&addr;
        ip_port->ip.family = (uint8_t)addr_in6->sin6_family;
...
static uint8_t at_shutdown_ran = 0;
static void networking_at_shutdown(void)
{
    if (at_shutdown_ran != 0)
        return;
    ...
    if (at_startup_ran != 0)
    {
        ...
        //    LOGGER_ERROR("WSACleanup error: %d", WSAGetLastError());
#endif
    }
...
void kill_networking(Networking_Core *net)
{
    ...
    if (net->family != 0) /* Socket was initialized */
        kill_sock(net->sock);

    networking_at_shutdown();
