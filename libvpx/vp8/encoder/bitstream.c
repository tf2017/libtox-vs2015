...
static int independent_coef_context_savings(VP8_COMP *cpi)
{
    ...
    do
    {
        ...
        {
            int k = 0;
            unsigned int prev_coef_count_sum[MAX_ENTROPY_TOKENS]; /* -C99 = {0} */
            int prev_coef_savings[MAX_ENTROPY_TOKENS]; /* -C99 = {0}; */
            const unsigned int (*probs)[MAX_ENTROPY_TOKENS];

            memset(prev_coef_count_sum, 0, sizeof(prev_coef_count_sum));
            memset(prev_coef_savings, 0, sizeof(prev_coef_savings));
...
void vp8_update_coef_probs(VP8_COMP *cpi)
{
    ...
    do
    {
        ...
        {
            int k = 0;
            int prev_coef_savings[ENTROPY_NODES]; /* -C99 = {0}; */
            memset(prev_coef_savings, 0, sizeof(prev_coef_savings));
            if (cpi->oxcf.error_resilient_mode & VPX_ERROR_RESILIENT_PARTITIONS)
            {
                for (k = 0; k < PREV_COEF_CONTEXTS; ++k)
