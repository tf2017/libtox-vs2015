...
void vp9_choose_segmap_coding_method(VP9_COMMON *cm, MACROBLOCKD *xd) {
  ...
  int temporal_predictor_count[PREDICTION_PROBS][2]; // -C99 = { { 0 } };
  int no_pred_segcounts[MAX_SEGMENTS]; // -C99 = { 0 };
  int t_unpred_seg_counts[MAX_SEGMENTS]; // -C99 = { 0 };

  vpx_prob no_pred_tree[SEG_TREE_PROBS];
  vpx_prob t_pred_tree[SEG_TREE_PROBS];
  vpx_prob t_nopred_prob[PREDICTION_PROBS];

  memset(temporal_predictor_count, 0, sizeof(temporal_predictor_count));
  memset(no_pred_segcounts, 0, sizeof(no_pred_segcounts));
  memset(t_unpred_seg_counts, 0, sizeof(t_unpred_seg_counts));
