...
static int64_t handle_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  uint8_t skip_txfm[MAX_MB_PLANE << 2]; // -C99 = {0};
  int64_t bsse[MAX_MB_PLANE << 2]; // -C99 = {0};
  ...
  memset(skip_txfm, 0, sizeof(skip_txfm));
  memset(bsse, 0, sizeof(bsse));
...
void vp9_rd_pick_inter_mode_sb(VP9_COMP *cpi,
  ...
  int best_skip2 = 0;
  uint8_t ref_frame_skip_mask[2] = { 0, 0 }; // -C99
  uint16_t mode_skip_mask[MAX_REF_FRAMES] = { 0, 0, 0, 0 }; // -C99
...
void vp9_rd_pick_inter_mode_sub8x8(VP9_COMP *cpi,
  ...
  int best_skip2 = 0;
  int ref_frame_skip_mask[2] = { 0, 0 }; // -C99
  int64_t mask_filter = 0;
  int64_t filter_cache[SWITCHABLE_FILTER_CONTEXTS];
